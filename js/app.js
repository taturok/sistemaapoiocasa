...
function injetarHTMLDinamico() {
  ...
  // Modal Editar Nível de Acesso
  if (!document.getElementById('modalEditarNivel')) {
    const modalNivel = document.createElement('div');
    modalNivel.className = 'modal-overlay';
    modalNivel.id = 'modalEditarNivel';
    modalNivel.innerHTML = `
      <div class="modal-box">
        <h2>✏️ Alterar Nível de Acesso</h2>
        <div class="campo">
          <label>Novo Nível</label>
          <select id="selectEditarNivel"></select>
        </div>
        <div class="modal-actions">
          <button class="btn btn-secondary" onclick="fecharModalEditarNivel()">Cancelar</button>
          <button class="btn btn-primary" onclick="salvarNivelEditado()">Salvar</button>
        </div>
      </div>
    `;
...
async function carregarLogo() {
  try {
    ...
    if (logoBase64) {
      ...
      // Também carregar na tela de login
...
function renderizarUsuarios() {
  ...
  if (!tbody) return;
  const podeEditarNivel = estado.usuarioAtual && ['admin', 'desenvolvedor', 'gestor'].includes(estado.usuarioAtual.nivel);
  tbody.innerHTML = estado.usuarios.filter(u => u.status === 'ativo').map(u => `
    <tr>
      <td>${u.nome || '-'}</td><td>${u.email || '-'}</td><td>${NIVEIS_ACESSO[u.nivel]?.nome || u.nivel || '-'}</td>
      <td style="color:#10b981;">${u.status}</td>
      <td>
        ${podeEditarNivel && u.id !== 'admin001' ? `<button onclick="editarNivelUsuario('${u.id}')" class="btn-acao" style="background:#3b82f6; margin-right:4px;">✏️ Nível</button>` : ''}
...
let userParaEditarNivel = null;

function editarNivelUsuario(id) {
  userParaEditarNivel = estado.usuarios.find(u => u.id === id);
  if (!userParaEditarNivel) return;
  const select = document.getElementById('selectEditarNivel');
  if (select) {
    select.innerHTML = Object.entries(NIVEIS_ACESSO)
      .filter(([k]) => k !== 'desenvolvedor')
      .map(([k, v]) => `<option value="${k}" ${userParaEditarNivel.nivel === k ? 'selected' : ''}>${v.nome}</option>`).join('');
  }
  document.getElementById('modalEditarNivel').style.display = 'flex';
}

async function salvarNivelEditado() {
  if (!userParaEditarNivel) return;
  const novoNivel = document.getElementById('selectEditarNivel').value;
  if (!novoNivel) return alert('Selecione um nível.');
  userParaEditarNivel.nivel = novoNivel;
  try {
    await upstash('SET', `user:${userParaEditarNivel.id}`, JSON.stringify(userParaEditarNivel));
    document.getElementById('modalEditarNivel').style.display = 'none';
    userParaEditarNivel = null;
    await carregarTodosDados();
    alert(`Nível de ${userParaEditarNivel?.nome || 'usuário'} alterado para ${NIVEIS_ACESSO[novoNivel]?.nome || novoNivel}.`);
  } catch (err) {
    alert('Erro: ' + err.message);
  }
}
...
function renderizarDashboard() {
  ...
  // Ativos = jovens com medida ativa E saldo > 0
  const ativos = estado.jovens.filter(j => {
    if (!j['MEDIDA'] || j['MEDIDA'] === 'Liberação') return false;
    return parseFloat(calcularSaldo(j)) > 0;
  }).length;
  // Liberados = medida formalmente liberada OU saldo zerado (concluído)
...
function renderizarGraficos() {
  ...
  // GRÁFICO DE EVOLUÇÃO DO STATUS AO LONGO DO TEMPO
  const ctx6 = document.getElementById('graficoEvolucao')?.getContext('2d');
  if (ctx6) {
    // Coletar todas as datas de frequência (entradas)
    const todasDatas = [];
    estado.jovens.forEach(j => {
      (j.historicoFrequencia || []).forEach(h => {
        if (h.tipo === 'entrada') {
          todasDatas.push(new Date(h.data));
        }
      });
    });
    ...
    todasDatas.forEach(d => {
      ...
      if (!mesesMap[key]) mesesMap[key] = { ativos: 0, concluidos: 0, liberados: 0, totalRegistros: 0 };
      mesesMap[key].totalRegistros++;
    });

    // Para cada mês, calcular quantos jovens tinham status ativo, concluído ou liberado naquele período
    ...
    const concluidosArr = [];
    const liberadosArr = [];
    let totalAtivos = 0, totalConcluidos = 0, totalLiberados = 0;

    meses.forEach(mes => {
      // Simular o estado ao final de cada mês
      const [mm, yyyy] = mes.split('/').map(Number);
      const fimDoMes = new Date(yyyy, mm, 0, 23, 59, 59);

      let ativos = 0, concluidos = 0, liberados = 0;
      estado.jovens.forEach(j => {
        if (j['MEDIDA'] === 'Liberação') {
          // Verificar se a liberação foi antes do fim do mês
          liberados++;
        } else {
          // Calcular saldo daquele jovem até a data fimDoMes
          const histAteMes = (j.historicoFrequencia || []).filter(h => new Date(h.data) <= fimDoMes);
          const horasFeitasAteMes = histAteMes.reduce((s, h) => s + parseNum(h.horas), 0);
          const horasTotal = parseNum(j['HORAS']);
          const saldo = Math.max(0, horasTotal - horasFeitasAteMes);
          if (saldo <= 0) concluidos++;
          else ativos++;
        }
      });

      ativosArr.push(ativos);
      concluidosArr.push(concluidos);
      liberadosArr.push(liberados);
    });

    estado.graficos.evolucao = new Chart(ctx6, {
      type: 'line',
      data: {
        labels: meses,
        datasets: [
          { label: 'Ativos', data: ativosArr, borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,0.1)', fill: true, tension: 0.3 },
          { label: 'Concluídos', data: concluidosArr, borderColor: '#f59e0b', backgroundColor: 'rgba(245,158,11,0.1)', fill: true, tension: 0.3 },
          { label: 'Liberados', data: liberadosArr, borderColor: '#6b7280', backgroundColor: 'rgba(107,114,128,0.1)', fill: true, tension: 0.3 }
        ]
      },
      options: {
        responsive: true,
...
function carregarLista() {
  ...
  tbody.innerHTML = lista.map(j => {
    ...
    if (j['MEDIDA'] === 'Liberação') {
      status = 'liberado'; statusClass = 'ponto-status fechado';
    } else if (saldoNum <= 0) {
      status = 'concluído'; statusClass = 'ponto-status fechado';
    } else if (hist.length > 0) {
      status = 'ativo'; statusClass = 'ponto-status entrada';
    } else {
      status = 'inativo'; statusClass = 'ponto-status saida';
    }

    // Verificar se há entrada aberta hoje para mostrar estado do botão
    const hoje = new Date();
    const hojeStr = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate()).getTime();
    let temEntradaAberta = false;
    let podeRegistrarPonto = j['MEDIDA'] !== 'Liberação' && saldoNum > 0;
    if (podeRegistrarPonto) {
      for (let i = hist.length - 1; i >= 0; i--) {
        if (hist[i].tipo === 'entrada') {
          const dataEntrada = new Date(hist[i].data);
          const entradaDia = new Date(dataEntrada.getFullYear(), dataEntrada.getMonth(), dataEntrada.getDate()).getTime();
          if (entradaDia === hojeStr) { temEntradaAberta = true; break; }
        }
        if (hist[i].tipo === 'saida') {
          const dataSaida = new Date(hist[i].data);
          const saidaDia = new Date(dataSaida.getFullYear(), dataSaida.getMonth(), dataSaida.getDate()).getTime();
          if (saidaDia === hojeStr) break; // saída hoje = sem entrada aberta
        }
      }
    }
    ...
    return `<tr>
      ...
      <td>
        ${podeRegistrarPonto ? `<button onclick="registrarPontoNaLinha('${j.id}')" class="btn-acao ${temEntradaAberta ? 'btn-ponto-saida' : 'btn-ponto-entrada'}" title="${temEntradaAberta ? 'Registrar Saída' : 'Registrar Entrada'}">${temEntradaAberta ? '🚪 Saída' : '🚪 Entrada'}</button>` : '<span style="color:#9ca3af; font-size:0.8rem; padding:4px;">' + (status === 'concluído' ? '✅ Concluído' : '—') + '</span>'}
...
// ================================================================
// PONTO NA LINHA DA TABELA (toggle entrada/saída por jovem)
// ================================================================
async function registrarPontoNaLinha(jovemId) {
  ...
  // Verificar se há entrada aberta hoje
  ...
  for (let i = hist.length - 1; i >= 0; i--) {
    if (hist[i].tipo === 'entrada') {
      const dataEntrada = new Date(hist[i].data);
      const entradaDia = new Date(dataEntrada.getFullYear(), dataEntrada.getMonth(), dataEntrada.getDate()).getTime();
      if (entradaDia === hojeStr) { entradaAberta = hist[i]; break; }
    }
    if (hist[i].tipo === 'saida') {
      const dataSaida = new Date(hist[i].data);
      const saidaDia = new Date(dataSaida.getFullYear(), dataSaida.getMonth(), dataSaida.getDate()).getTime();
      if (saidaDia === hojeStr) break;
    }
  }

  if (entradaAberta) {
    // Registrar SAÍDA com horas reais
    const dataEntrada = new Date(entradaAberta.data);
    const diffMs = now.getTime() - dataEntrada.getTime();
...
// ================================================================
// PONTO DIGITAL — ENTRADA E SAÍDA COM HORAS REAIS (via leitor digital)
// ================================================================
async function registrarPontoDigital() {
  ...
  // Verificar se há uma entrada aberta (sem saída correspondente)
  // Entrada aberta = registro do tipo 'entrada' que ainda não tem um registro 'saída' no mesmo dia
  let entradaAberta = null;
  for (let i = hist.length - 1; i >= 0; i--) {
    if (hist[i].tipo === 'entrada') {
      const dataEntrada = new Date(hist[i].data);
      const hoje = new Date(now.getFullYear(), now.getMonth(), now.getDate());
      const dataEntradaDia = new Date(dataEntrada.getFullYear(), dataEntrada.getMonth(), dataEntrada.getDate());
      if (dataEntradaDia.getTime() === hoje.getTime()) {
        entradaAberta = hist[i];
        break;
      }
    }
  }
  ...
  if (entradaAberta) {
    // Se há entrada aberta hoje, registra SAÍDA e calcula horas reais
    const dataEntrada = new Date(entradaAberta.data);
    const diffMs = now.getTime() - dataEntrada.getTime();
    const horasReais = diffMs / (1000 * 60 * 60);
    const horasArredondadas = Math.round(horasReais * 4) / 4; // Arredonda para 0.25h (15min)

    // Atualizar horas na entrada
    entradaAberta.horas = parseFloat(horasArredondadas.toFixed(2));
    entradaAberta.horaSaida = now.toISOString();
    ...
    msg.textContent = `✅ ${jovem['NOME']} - Saída registrada às ${now.toLocaleTimeString('pt-BR')} (${horasArredondadas.toFixed(2)}h)`;
    msg.style.color = '#f59e0b';
  } else {
    // Se não há entrada aberta hoje, registra ENTRADA
    hist.push({ data: now.toISOString(), horas: 0, tipo: 'entrada', observacao: '' });
    const msg = document.getElementById('mensagemPonto');
  ...
  if (conteudo) {
    conteudo.innerHTML = `
      ...
      <div class="secao-historico">
        <h4>Frequência (${(jovem.historicoFrequencia || []).length} registros) | Total: ${calcularSaldo(jovem)}h cumpridas</h4>
        <ul>${(jovem.historicoFrequencia || []).map(h => {
...
// ================================================================
// REGISTRO MANUAL — ENTRADA/SAÍDA OU HORAS DIRETAS
// ================================================================
async function salvarRegistroManual() {
  ...
  // Registrar entrada com as horas informadas (registro direto)
  ...
  // Se horas > 0, também registrar saída correspondente
  if (horas > 0) {
  ...
  try {
    ...
    carregarLista(); renderizarDashboard();
    alert(`Registro salvo: ${horas}h para ${jovem['NOME'] || jovem['REFERENCIA']}`);
  } catch (err) { alert('Erro: ' + err.message); }
}
...
function imprimirFichaIndividual() {
  ...
  printWindow.document.write(`<!DOCTYPE html><html><head><title>Ficha - ${jovem['NOME'] || jovem['REFERENCIA']}</title><style>
    ...
    .print-header img { max-height: 40px; max-width: 80px; object-fit: contain; }
  ...
  <h2>Frequência</h2>
  <p>Total de registros: ${hist.length} | Total de horas cumpridas: ${hist.reduce((s, h) => s + parseNum(h.horas), 0).toFixed(1)}h | Saldo: ${calcularSaldo(jovem)}h</p>
  ${hist.length > 0 ? `<table><thead><tr><th>Tipo</th><th>Data/Hora</th><th>Horas</th><th>Observação</th></tr></thead><tbody>${hist.map(h => `<tr><td>${h.tipo === 'saida' ? 'Saída' : 'Entrada'}</td><td>${new Date(h.data).toLocaleDateString('pt-BR')} ${new Date(h.data).toLocaleTimeString('pt-BR', {hour:'2-digit',minute:'2-digit'})}</td><td>${h.tipo === 'saida' ? '-' : (parseNum(h.horas) || 0) + 'h'}</td><td>${h.observacao || '-'}</td></tr>`).join('')}</tbody></table>` : '<p>Sem registros de frequência.</p>'}
  <h2>Oficinas Participadas (${oficinasParticipadas.length})</h2>
  ${oficinasParticipadas.length > 0 ? `<table><thead><tr><th>Data</th><th>Período</th><th>Conteúdo</th><th>Benefício Social</th></tr></thead><tbody>${oficinasParticipadas.map(o => `<tr><td>${new Date(o.data).toLocaleDateString('pt-BR')}</td><td>${o.periodo}</td><td>${o.conteudo}</td><td>${o.reverte ? '✅ Sim' : 'Não'}</td></tr>`).join('')}</tbody></table>` : '<p>Nenhuma oficina registrada.</p>'}
